...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    s3_client.upload_file(
        subtitle_output_path, "ai-podcast-clipper-77", output_s3_key)
...
class AiPodcastClipper:
    ...
    def identify_moments(self, transcript: dict):
        response = self.gemini_client.models.generate_content(model="gemini-2.0-flash", contents="""
    ...
    - Each clip should be between 20 and 60 seconds long.
    - If a clip is shorter than 20 seconds, DO NOT include it.
    - The clips should be coherent and self-contained, focusing on a single question and its answer
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("ai-podcast-clipper-77",
